...
class EquInput(ttk.Frame):
    def __init__(self, main_win, *args, **kwargs):
        ...
        self.equ_entry = ttk.Combobox(self,
                                      exportselection=0,
                                      width=100,
                                      values=list(self.main_win.equations.keys()))

        self.insert_btn.grid(row=0, column=0, padx=self.main_win.padx, pady=self.main_win.pady)
        self.cell_entry.grid(row=0, column=1, padx=self.main_win.padx, pady=self.main_win.pady)
        self.equal_lab.grid(row=0, column=2, padx=self.main_win.padx, pady=self.main_win.pady)
        self.equ_entry.grid(row=0, column=3, padx=self.main_win.padx, pady=self.main_win.pady)
    ...
    def insert_equ(self, *args):
        ...
        if not equ_str:
            return
        result = self.main_win.compute_equation(equ_str)
        cell_id = self.cell_entry.get().upper()
        try:
            self.main_win.cell_area.set_cell_content(cell_id, result)
        except KeyError:
            self.cell_entry.configure(background="red", foreground="white")
            return
        self.main_win.cell_area.set_cell_equ(cell_id, self.equ_entry.get())
